@LinearEditorView = Backbone.View.extend
  className: "modal"
  ...
  events:
    ...
    "click #mark": "mark"
    "click #update": "update"
    "change #linear-form-color": "getColor"
    
  getColor: (event) =>
    color = $("#linear-form-color").val()
  ...
  update: () ->
    @model.set("full_title", $('#linear-form-full-title').val())
    @model.set("color", $('#linear-form-color').val())
    @model.set("content", $('#linear-form-content').val())
  ...
  handle: () ->
    socket.authRegisterPrivate "time_marker_linear_#{@model.get('item_id')}", (data) =>
      if(!@ready)
        @ready=true
    ...
    socket.authRegisterPrivate "time_marker_linear_time_#{@model.get('item_id')}", (data) =>
...
#                           
  ...
  render: () ->
    tmp = app.template('linearForm','story', @model.toJSON())
    @$el.html(tmp)
    @$el.modal
      backdrop: "static"
